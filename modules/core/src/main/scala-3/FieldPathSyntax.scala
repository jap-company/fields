...
trait FieldPathSyntax {
  extension (path: FieldPath.type) {
    ...
    /** Returns [[fields.FieldPath]] inferered from field selects
      ...
      * }}}
      */
    inline def from[V](inline value: V): FieldPath = ${
      FieldPathMacro.fromImpl[V]('value, includeIdent = true, title = "FieldPath.from")
    }
    ...
    /** Similar to `Field.from` but drops first selector path
      ...
      * }}}
      */
    inline def sub[V](inline value: V): FieldPath = ${
      FieldPathMacro.fromImpl[V]('value, includeIdent = false, title = "FieldPath.sub")
    }
  }
}
