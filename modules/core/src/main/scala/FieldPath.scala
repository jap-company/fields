...
final class FieldPath private (val parts: List[FieldPart]) extends AnyVal with Serializable {
  ...
  def full: String = if (parts.isEmpty) FieldPath.RootName else parts.map(_.render).mkString
  ...
  def ++:(path: FieldPath): FieldPath = FieldPath(path.parts ++ parts)

  def +:(part: FieldPart): FieldPath = FieldPath(part :: parts)
  ...
  def downKey(key: String): FieldPath = FieldPath(parts :+ FieldPart.Key(key))
...
object FieldPath extends FieldPathSyntax {
  ...
  /** Root FieldPath */
  val Root: FieldPath = FieldPath()

  /** Create from List */
  def apply(parts: List[FieldPart]): FieldPath = new FieldPath(parts)
  ...
  /** Conversion from value.Field */
  implicit def fromField[P](f: value.Field[P]): FieldPath = f.path

  /** Conversion from lens.FieldLens */
  implicit def fromFieldLens[P, V](f: lens.FieldLens[P, V]): FieldPath = f.path

  private val IndexRegex = "\\[(\\d+)\\]".r

  /** Parse `FieldPath` from dot-separated `path` string */
  def parse(path: String): FieldPath = FieldPath(
    path
      .split('.')
      .flatMap { part =>
        if (part.isEmpty) Nil
...
sealed trait FieldPart extends Any with Serializable {
  def render: String
  def name: String
}
object FieldPart {
  case class Key(value: String)  extends AnyVal with FieldPart {
    def render: String = s"[$value]"
    def name: String   = value
  }
  case class Path(value: String) extends AnyVal with FieldPart {
    def render: String = "." + value
    def name: String   = value
  }
  case class Index(value: Int)   extends AnyVal with FieldPart {
    def render: String = s"[$value]"
...
object FieldPartConversions {
  ...
  /** Conversion from `String` to [[fields.FieldPart.Path]] */
  implicit def fromPath(path: String): FieldPart = FieldPart.Path(path)

  /** Conversion from `String` to [[fields.FieldPart.Index]] */
