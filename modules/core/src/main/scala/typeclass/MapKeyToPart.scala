...
object MapKeyToPart extends MapKeyToPartInstances0 {
  implicit val stringMapKeyToPart: MapKeyToPart[String] = (key, _) => FieldPart.Key(key)
  implicit val intMapKeyToPart: MapKeyToPart[Int]       = (key, _) => FieldPart.Key(key.toString)
}
