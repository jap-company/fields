...
final class BooleanFieldOps[F[_], V[_], E](private val field: Field[Boolean]) extends AnyVal {
  ...
  def isTrue(implicit R: RuleOps[F, V, E], FW: FailWithCompare[E, Boolean]): RuleK[F, V, E] =
    field.assert(_ == true, FW.equal(true))

  /** Validates [[fields.Field]]#value is `false` */
  def isFalse(implicit R: RuleOps[F, V, E], FW: FailWithCompare[E, Boolean]): RuleK[F, V, E] =
