...
final class StringFieldOps[F[_], V[_], E](private val field: Field[String]) extends AnyVal {
  ...
  def startsWith(
      value: => String
  )(implicit R: RuleOps[F, V, E], FW: FailWithMessage[E, String]): RuleK[F, V, E] =
  ...
  def nonEmpty(implicit R: RuleOps[F, V, E], FW: FailWithNonEmpty[E, String]): RuleK[F, V, E] =
    field.assert(_.nonEmpty, FW.nonEmpty)

  /** Validates that [[fields.Field]]#value is not blank */
  def nonBlank(implicit R: RuleOps[F, V, E], FW: FailWithNonEmpty[E, String]): RuleK[F, V, E] =
    field.assert(_.nonEmpty, FW.nonEmpty)

  /** Validates that [[fields.Field]]#value is blank */
  def blank(implicit R: RuleOps[F, V, E], FW: FailWithEmpty[E, String]): RuleK[F, V, E] =
    field.assert(_.isEmpty, FW.empty)

  /** Validates that [[fields.Field]]#value minimum size is `min` */
  def minSize(min: => Int)(implicit R: RuleOps[F, V, E], FW: FailWithMinSize[E, String]): RuleK[F, V, E] =
    field.assert(_.length >= min, FW.minSize(min))

  /** Validates that [[fields.Field]]#value maximum size is `max` */
  def maxSize(max: => Int)(implicit R: RuleOps[F, V, E], FW: FailWithMaxSize[E, String]): RuleK[F, V, E] =
    field.assert(_.length <= max, FW.maxSize(max))

  /** Validates that [[fields.Field]]#value matches Regexp */
  def matches(r: => String)(implicit R: RuleOps[F, V, E], FW: FailWithMessage[E, String]): RuleK[F, V, E] =
  ...
  def matchesRegex(r: => Regex)(implicit R: RuleOps[F, V, E], FW: FailWithMessage[E, String]): RuleK[F, V, E] =
    matches(r.regex)

  /** Validates that [[fields.Field]]#value is part of [[scala.Enumeration]] */
  def isEnum(e: Enumeration)(implicit R: RuleOps[F, V, E], FW: FailWithOneOf[E, String]): RuleK[F, V, E] =
    field.assert(
  ...
  def isJEnum[T <: Enum[T]](
      values: Array[T]
  )(implicit R: RuleOps[F, V, E], FW: FailWithOneOf[E, String]): RuleK[F, V, E] =
